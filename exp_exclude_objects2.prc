------------------------------------------------------------------------------------
-- File name: exp_exclude_objects2.prc
-- Purpose:   To demonstrate a schema export that excludes certain objects.
-- Author:    Christoph Ruepprich
--            http://ruepprich.wordpress.com
--            cruepprich@gmail.com
-- Notes:     For educational purposes only.
--            The procedure will export the SCOTT schema, but exclude
--            indexes, statistics, and constraints.
------------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE exp_exclude_objects2 IS
...
BEGIN
  l_job_name := 'exp_exclude_objects2';
...
/*
  BEGIN -- exclude some objects
    dbms_datapump.metadata_filter(handle => l_dp_handle,
                                  NAME   => 'EXCLUDE_PATH_LIST',
                                  VALUE  => 'TABLE/INDEX','TABLE/STATISTICS', 'TABLE/CONSTRAINT');
